...
const darken = (hex: string, amt = 0.14): string => {
  ...
  const to = (c: number) => Math.max(0, Math.min(255, Math.round(c * (1 - amt))));
...
export default function SetDetailsDrawer({ open, onClose, setId }: SetDetailsDrawerProps) {
  ...
  const [setInfo, setSetInfo] = useState<TcgSetInfo | null>(null);
  const [loadingSet, setLoadingSet] = useState(false);
  const [setErr, setSetErr] = useState<unknown>(null);

  const [cards, setCards] = useState<TcgDrawerCard[]>([]);
  ...
  useEffect(() => {
    ...
    axios
      .get<{ data: TcgSetInfo }>(`${API_BASE}/sets/${setId}`, axiosCfg)
      .then((res) => !cancelled && setSetInfo(res.data?.data ?? null))
      .catch((err) => {
        if (!cancelled) {
          setSetErr(isAxiosError(err) ? err : err);
        }
      })
  ...
  useEffect(() => {
    ...
    axios
      .get<{ data: TcgDrawerCard[]; totalCount?: number; total?: number }>(
      ...
      .catch((err) => {
        if (!cancelled) {
          setCardsErr(isAxiosError(err) ? err : err);
        }
      })
      ...
      <Box sx={{ px: { xs: 2, md: 3 }, py: { xs: 2, md: 3 } }}>
        ...
        {cardsErr ? (
          <Card sx={{ p: 2, bgcolor: redSofter, border: `1px solid ${redLine}` }}>
            <Typography color="error" sx={{ mb: 1, fontWeight: 700 }}>
              Couldn’t load cards: {getErrorMessage(cardsErr)}
            </Typography>
            <Button size="small" variant="outlined" sx={{
              borderColor: POKE_RED, color: POKE_RED, '&:hover': { borderColor: darken(POKE_RED), background: alpha(POKE_RED, .06) }
            }} onClick={() => setPage((p) => p)}>
